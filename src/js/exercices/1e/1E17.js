...
export const titre = 'Utiliser les différentes formes d\'un polynome du second degré'
...
export default class EtudeTrinome extends Exercice {
  ...
  nouvelleVersion () {
    ...
    this.introduction = `Soit $f$ la fonction définie sur $\\R$ par $f(x)=${p.texFormeCanonique}$ (forme 1).`
    const question1 = 'Développer $f(x)$ (forme 2).'
    ...
    const question2 = `Montrer que $f(x)$ se factorise sous la forme $f(x)=${p.texFormeFactorisee}$ (forme 3).`
    ...
    let question3 = 'Répondre aux questions suivantes en utilisant la forme de $f$ la mieux adaptée :'
    let correction3 = ''

    const q3a = 'Résoudre l\'équation $f(x) = 0$.'

    let corr3a = 'Ici, on va utiliser la forme factorisée (forme 3) pour se ramener à une équation produit. Il est aussi possible de calculer le discriminant mais cela serait plus long.'
    ...
    let corr3b = 'Ici, on va utiliser la forme développée (forme 2).'
    ...
    const q3c = `Résoudre l'inéquation $f(x)\\leq ${p.beta.simplifie().texFraction}$.`
    let corr3c = 'Ici, on va utiliser la forme canonique (forme 1).'
    corr3c += `<br><br>$f(x)\\leq ${p.beta.simplifie().texFraction} \\iff ${p.texFormeCanonique} \\leq ${p.beta.simplifie().texFraction}$`
    corr3c += `<br><br>$\\phantom{f(x)\\leq ${p.beta.simplifie().texFraction}} \\iff ${p.a.simplifie().texFractionSaufUn}\\left( x ${p.alpha.oppose().simplifie().texFractionSignee} \\right)^2 \\leq 0$`
    corr3c += `<br><br>$${p.a.simplifie().texFraction} ${p.a.s === 1 ? '>0' : '<0'}$ et $\\left( x ${p.alpha.oppose().simplifie().texFractionSignee} \\right)^2 \\geq 0$`
