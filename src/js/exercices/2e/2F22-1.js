...
import { listeQuestionsToContenu, randint, reduireAxPlusB, abs, pgcd, texFractionReduite, texteEnCouleurEtGras, texFraction, miseEnEvidence, ecritureAlgebrique } from '../../modules/outils.js'
import { repere2, droite, mathalea2d, point, tracePoint, labelPoint, segment, texteParPosition, latexParPoint, similitude, vecteur, translation, homothetie } from '../../modules/2d.js'
...
export default function lecturefonctionaffine () {
  ...
  this.nouvelleVersion = function () {
    ...
    for (let i = 0, A, B, a, b, d, r, f, c, t, l, s1, s2, texte, texteCorr, cpt = 0;
      ...
      if (this.sup === 2) { // cas du coeff directeur fractionnaire
        ...
        if (a === d) { texteCorr += 'x' }
        if (a === -d) { texteCorr += '-x' }
        if (a !== d & a !== -d) { texteCorr += `${texFraction(miseEnEvidence(a, 'green'), miseEnEvidence(d, 'blue'))}x` }
