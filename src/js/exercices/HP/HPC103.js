...
export const titre = 'Calculs de probabilité avec la loi exponentielle'
...
export default function CalculsLoiExponentielle () {
  Exercice.call(this) // Héritage de la classe Exercice()
  this.titre = titre
  this.consigne = 'Calcul de probabilités pour une loi exponentielle. Les valeurs approchées seront données avec une précision de $10^{-2}$.'
  this.nbQuestions = 2
  ...
  this.nouvelleVersion = function () {
    ...
    if (this.sup === 1) {
      listeTypeDeQuestionsDisponibles = ['loiexp']
    } else if (this.sup === 2) {
      listeTypeDeQuestionsDisponibles = ['loiexpcontexte']
    } else {
      listeTypeDeQuestionsDisponibles = ['loiexp']
    }
    function dexponentielle (x, param) {
      let y
      if (x < 0) { y = 0 } else { y = param * math.exp(-x * param) }
      return y
    }
    const listeTypeDeQuestions = combinaisonListes(listeTypeDeQuestionsDisponibles, this.nbQuestions)
    for (let i = 0, texte, texteCorr, variables, expression, densite, r, C, I, graphique, resultat, resultatA, resultatB, bornea, oppbornea, borneb, oppborneb, mu, sigma, bornec, borned, calculstep, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      switch (listeTypeDeQuestions[i]) {
        ...
        case 'loiexpcontexte':
          ...
          densite = x => 1 / variables.sigma / math.sqrt(2 * math.pi) * math.exp(-((x - variables.mu) ** 2) / 2 / (variables.sigma ** 2))
          ...
          C = courbe2(densite, { repere: r, step: 0.1 })
          I = integrale(densite, { repere: r, step: 0.1, a: variables.a * variables.sigma + variables.mu, b: variables.b * variables.sigma + variables.mu, hachures: 0 })
