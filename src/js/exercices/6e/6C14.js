...
import { listeQuestionsToContenu, range, combinaisonListesSansChangerOrdre, randint, texteEnCouleurEtGras, listeDesDiviseurs, sp, numAlpha, contraindreValeur, choice, arrondi, prenomF, rangeMinMax, texNombre, texNombre3, troncature, estentier, compteOccurences, enleveDoublonNum, enleveDoublonNum2, combinaisonListes } from '../../modules/outils.js'
    ...
    for (let i = 0, texte, texteCorr, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      ...
      if (!this.sup) { // Si aucune liste n'est saisie
        ...
        if (typeof (this.sup) === 'number') { // Si c'est un nombre c'est que le nombre a été saisi dans la barre d'adresses
          QuestionsDisponibles[0] = contraindreValeur(1, 8, this.sup, 9)
        } else {
      ...
      let PrixReduction
      if (this.sup3) {
        PrixUnitaire = arrondi(10 * TabPrixUnitaire[0] + TabPrixUnitaire[1], 0)
        AutrePrix = arrondi(10 * TabAutrePrix[0] + TabAutrePrix[1], 0)
        PrixReduction = arrondi(choice(rangeMinMax(11, arrondi(PrixUnitaire / 2, 0)), 0))
      } else {
      ...
      texte = `${quidame} repère un article dans un magazine de publicité à ${texNombre3(PrixUnitaire)}&nbsp;€. <br><br>`
      // listeQuestions[0] = 1 // A SUPPRIMER
      for (let kk = 0; kk < QuestionsDisponibles.length; kk++) {
        switch (QuestionsDisponibles[kk]) {
          case 1:
            if (this.interactif && !context.isAmc) {
              texte = `${quidame} repère un article dans un magazine de publicité à $${texNombre(PrixUnitaire)}$ toto&nbsp;€. <br>`
              ...
              texte += numAlpha(kk) + `Quel serait le prix de ${DixOuCent} articles à ${texNombre3(PrixUnitaire)}&nbsp;€ l'unité&nbsp;?<br><br>`
              texteCorr = numAlpha(kk)
              texteCorr += ` $${DixOuCent} \\times ${texNombre3(PrixUnitaire)} = ${texNombre3(DixOuCent * PrixUnitaire)}$<br>`
              texteCorr += `Le prix de ${DixOuCent} articles serait de ${texNombre3(DixOuCent * PrixUnitaire)}&nbsp;€.<br><br>`
            }
            break
          case 2:
            texte += numAlpha(kk) + `Quel serait le prix de ${NbArticles} articles à ${texNombre3(PrixUnitaire)}&nbsp;€ l'unité&nbsp;?<br><br>`
            texteCorr += numAlpha(kk)
            texteCorr += ` $${NbArticles} \\times ${texNombre3(PrixUnitaire)} = ${texNombre3(arrondi(NbArticles * PrixUnitaire))}$<br>`
            texteCorr += `Le prix de ${NbArticles} articles serait de ${texNombre3(arrondi(NbArticles * PrixUnitaire))}&nbsp;€.<br><br>`
            break
          case 3:
            texte += numAlpha(kk) + `Si ${quidame} achetait un seul article à ${texNombre3(PrixUnitaire)}&nbsp;€ ainsi que d'autres articles pour ${texNombre(AutrePrix)}&nbsp;€, quel serait le prix final&nbsp;?<br><br>`
            texteCorr += numAlpha(kk)
            texteCorr += ` $${PrixUnitaire} + ${texNombre3(AutrePrix)} = ${texNombre3(PrixUnitaire + AutrePrix)}$<br>`
            texteCorr += `Si ${quidame} achetait un seul article à ${texNombre3(PrixUnitaire)}&nbsp;€ ainsi que d'autres articles pour ${texNombre(AutrePrix)}&nbsp;€, `
            texteCorr += `le prix final serait de ${texNombre3(PrixUnitaire + AutrePrix)}&nbsp;€.<br><br>`
            break
          case 4:
            texte += numAlpha(kk) + `${quidame} dispose d'un bon de réduction de ${texNombre3(PrixReduction)}&nbsp;€. Si ${quidame} achetait un seul article à ${texNombre(PrixUnitaire)}&nbsp;€, quel somme d'argent paierait ${quidame} au final&nbsp;?<br><br>`
            texteCorr += numAlpha(kk)
            texteCorr += ` $${PrixUnitaire} - ${texNombre3(PrixReduction)} = ${texNombre3(PrixUnitaire - PrixReduction)}$<br>`
            texteCorr += `Grâce à son bon de réduction, ${quidame} ne paierait que ${texNombre3(PrixUnitaire - PrixReduction)}&nbsp;€.<br><br>`
            break
          case 5:
            texte += numAlpha(kk) + `Si ${quidame} achetait ${NbArticles2} articles à ${PrixUnitaire}&nbsp;€ l'unité et son ${Personnage1} en achetait ${NbArticles3} de plus, quelle somme paierait-ils à eux deux&nbsp;?<br><br>`
            ...
            texteCorr += `Si ${quidame} et son ${Personnage1} achetaient ${NbArticles2 + NbArticles3} articles à ${texNombre3(PrixUnitaire)}&nbsp;€ l'unité, `
            texteCorr += `le prix final serait de ${texNombre3(arrondi((NbArticles2 + NbArticles3) * PrixUnitaire))}&nbsp;€.<br><br>`
            break
          case 6:
            texte += numAlpha(kk) + `Si ${quidame} achetait ${NbArticles5} articles à ${PrixUnitaire}&nbsp;€ l'unité mais que sa ${Personnage2} lui propose de lui en rembourser ${NbArticles4}, quelle somme ${quidame} dépenserait-elle&nbsp;?<br><br>`
            texteCorr += numAlpha(kk)
            ...
            texteCorr += `Si ${quidame} achetait ${NbArticles5} articles à ${PrixUnitaire}&nbsp;€ l'unité mais que sa ${Personnage2} lui propose de lui en rembourser ${NbArticles4}, `
            texteCorr += `${quidame} dépenserait ${texNombre3(arrondi((NbArticles5 - NbArticles4) * PrixUnitaire))}&nbsp;€.<br><br>`
            break
          case 7 :
            texte += numAlpha(kk) + `Si ${quidame} décidait d'acheter cet article à ${PrixUnitaire}&nbsp;€ avec 9 amis, quelle somme équitable minimale devraient-ils, chacun, payer&nbsp;?<br><br>`
            texteCorr += numAlpha(kk)
            texteCorr += '$1 + 9 = 10$<br>'
            ...
            if (this.sup3) {
              ...
              texteCorr += `Si ${quidame} partageait cet article avec 9 amis, chacun paierait au minimum ${texNombre3(arrondi(PrixUnitaire / 10, 3))}&nbsp;€.<br><br>`
            } else {
              ...
              texteCorr += `Si ${quidame} partageait cet article avec ${Nbpartage} camarades, chacun paierait au minimum ${texNombre3(troncature(arrondi(PrixUnitaire / (Nbpartage + 1), 3) + 0.01, 2))}&nbsp;€.<br><br>`
            } break
          case 8 :
            texte += numAlpha(kk) + `Si ${quidame} décidait d'acheter cet article à ${PrixUnitaire}&nbsp;€ avec ${Nbpartage} camarades, quelle somme équitable minimale devraient-ils, chacun, payer&nbsp;?<br><br>`
            texteCorr += numAlpha(kk)
            ...
            if (estentier(arrondi(PrixUnitaire * 100, 0) / (Nbpartage + 1))) {
              ...
              texteCorr += `Si ${quidame} partageait cet article avec ${Nbpartage} camarades, chacun paierait au minimum ${texNombre3(arrondi(PrixUnitaire / (Nbpartage + 1), 3))}&nbsp;€.<br><br>`
            } else {
              texteCorr += `$${PrixUnitaire} \\div ${Nbpartage + 1} \\approx ${texNombre3(arrondi(PrixUnitaire / (Nbpartage + 1), 3))}$ et $${texNombre3(troncature(arrondi(PrixUnitaire / (Nbpartage + 1), 3), 2))} < ${texNombre(arrondi(PrixUnitaire / (Nbpartage + 1), 3))} < ${texNombre(troncature(arrondi(PrixUnitaire / (Nbpartage + 1), 3) + 0.01, 2))}$<br>`
              texteCorr += `Si ${quidame} partageait cet article avec ${Nbpartage} camarades, chacun paierait au minimum ${texNombre3(troncature(arrondi(PrixUnitaire / (Nbpartage + 1), 3) + 0.01, 2))}&nbsp;€.<br><br>`
            }
