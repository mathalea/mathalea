...
export default function ProblemesAiresRectangles () {
  ...
  function prepareProblemeAire (objetsEnonce, rectangles, typeDeGrille, longueursHorizontales, longueursVerticales, aires, xBordures, yBordures, nombreTotalEtapes) {
    const colonneDisponible = Array(typeDeGrille[0]).fill(true)
    const ligneDisponible = Array(typeDeGrille[1]).fill(true)
    let alternance
        ...
        case 'colonne':
          ...
          if (etape === 0) {
            ...
            if (etape === etapeAireInconnue) {
              objetsEnonce.push(latexParCoordonnees('?', (xBordures[x] + xBordures[x + 1]) / 2, (yBordures[y] + yBordures[y + 1]) / 2 + 0.5, 'black', 30, 10, '', 10))
            } else {
              objetsEnonce.push(latexParCoordonnees(aires[x][y].toString() + '\\text{ cm}^2', (xBordures[x] + xBordures[x + 1]) / 2, (yBordures[y] + yBordures[y + 1]) / 2 + 0.5, 'black', 30, 10, '', 10))
            }
          }
    ...
    if (alternance === 'ligne') {
      objetsEnonce.push(texteParPosition(longueursHorizontales[index].toString() + ' cm', (xBordures[x] + xBordures[x + 1]) / 2, yBordures[typeDeGrille[1]] + 0.5, 'milieu', 'black', 1, 'middle', false))
    } else {
      objetsEnonce.push(texteParPosition(longueursVerticales[index].toString() + ' cm', xBordures[typeDeGrille[0]] + 0.5, (yBordures[y] + yBordures[y + 1]) / 2, 'milieu', 'black', 1, 'middle', false))
    }
    let numeroteur = 0
    for (let j = 0; j < typeDeGrille[1]; j++) {
      for (let i = 0; i < typeDeGrille[0]; i++) {
  ...
  this.nouvelleVersion = function () {
    ...
    for (let q = 0, cpt = 0, texte, texteCorr; q < this.nbQuestions && cpt < 50;) {
      ...
      if (q % 2 === 0) {
        ...
        ;[texte, texteCorr] = prepareProblemeAire(objetsEnonce, rectangles, typeDeGrille, longueursHorizontales, longueursVerticales, aires, xBordures, yBordures, nombreTotalEtapes[q])
      }
