    ...
    for (let i = 0, a, b, n, typesDeQuestions, texte, texteCorr, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      ...
      a = choice(
        [
          ...
          randint(1, 9) * 1000 + randint(1, 9) * 10
        ])
      // X, XX, X0X, X00X, XX0
      b = choice([10, 100, 10, 100, 1000])
      n = calcul(a / b)
      ...
      switch (typesDeQuestions) {
        ...
        case 1: // / écriture décimale -> fraction décimale
          setReponse(this, i, a)
          if (this.interactif) {
            texte = `$${texNombre(n, nbdigits + randint(0, 3 - nbdigits), true)} =$` + ajouteChampFractionMathLive(this, i, false, b, 'min-width:100px')
          } else {
            texte = `$${texNombre(n, nbdigits + randint(0, 3 - nbdigits), true)} =$` + `$${texFraction('\\ldots\\ldots\\ldots\\ldots', texNombre(b))} $`
          }
