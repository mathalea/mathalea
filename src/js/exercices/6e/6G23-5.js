...
export default function MesurerUnAngleAvecRapporteur () {
  ...
  this.nouvelleVersion = function () {
    ...
    for (let i = 0, texte, texteCorr, reponse, A, B, B1, C, C1, D, D1, AB, AC, AD, sensRot, posA, posB, posC, posD, angB, angC, angD, paramsEnonce; i < this.nbQuestions; i++) {
      ...
      posA = angleModulo(angB - sensRot * 90) > 135 ? 'left' : (angleModulo(angB - sensRot * 90) > 45 ? 'above' : (angleModulo(angB - sensRot * 90) > -45 ? 'right' : (angleModulo(angB - sensRot * 90) > -135 ? 'below' : 'left')))
      ...
      objetsEnonce.push(AB, AC, AD, labelPoint(A, B, C, D), tracePoint(B, C, D), afficheMesureAngle(B, A, D, 'black', 2, '', { ecart: 0.75 })) // On remplit les tableaux d'objets Mathalea2d
      objetsCorrection.push(AB, AC, AD, labelPoint(A, B, C, D), tracePoint(B, C, D), afficheMesureAngle(B, A, D, 'black', 2, '', { ecart: 0.75 }), afficheMesureAngle(C, A, D, '#f15929', 2, '', { ecart: 0.75, colorArc: '#f15929', arcEpaisseur: 2, mesureEnGras: true })) // On remplit les tableaux d'objets Mathalea2d
      if (QuestionsDisponibles[i] === 1) {
        ...
        if (QuestionsDisponibles[i] === 2) objetsEnonce.push(afficheMesureAngle(B, A, C, 'blue', 1.5, '', { ecart: 0.75, saillant: sensRot === -1, colorArc: 'blue', arcEpaisseur: 2 })) // On remplit les tableaux d'objets Mathalea2d
