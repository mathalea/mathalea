...
import { premiereLettreEnMajuscule, listeQuestionsToContenuSansNumero, texcolors, arrondiVirgule, texFraction, combinaisonListes, tableauColonneLigne, choice, randint, rangeMinMax } from '../../modules/outils.js'
...
export const titre = 'Représenter des données par un diagramme (valeur exacte)'
...
export default function ConstruireUnDiagramme () {
  ...
  this.nouvelleVersion = function () {
    ...
    lstCoeffAnimaux.push(20)
    const max = Math.floor(20 / nbAnimaux)
    ...
    switch (listeTypeDeQuestions[0]) {
      case 1:
        ...
        for (let i = 0; i < nbAnimaux; i++) {
          contenutableau.push(texFraction(lstNombresAnimaux[i], effectiftotal) + '= ' + arrondiVirgule(lstNombresAnimaux[i] / effectiftotal, 2))
        }
        contenutableau.push('1')
        for (let i = 0; i < nbAnimaux; i++) {
          contenutableau.push(`${texFraction(lstNombresAnimaux[i], effectiftotal)} \\times 360 = ${Math.round(lstNombresAnimaux[i] * 360 / effectiftotal)}\\degree`)
        }
      ...
      case 2:
        ...
        contenutableau.push('1')
        for (let i = 0; i < nbAnimaux; i++) {
          contenutableau.push(`${texFraction(lstNombresAnimaux[i], effectiftotal)} \\times 180 = ${Math.round(lstNombresAnimaux[i] * 180 / effectiftotal)}\\degree`)
        }
