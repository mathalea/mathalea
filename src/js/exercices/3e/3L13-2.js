...
import {listeQuestionsToContenu,randint,choice,shuffle,combinaisonListesSansChangerOrdre,calcul,texNombre,texteEnCouleur_et_gras,tableauColonneLigne,warnMessage} from '../../modules/outils.js'
			...
			for (let k = 0; k < situations.length; k++) {
				...
				enonces.push({
					...
					correction: `${corrPlusPremiereLigne}
						$${situations[k].eq}$<br>
						${texteEnCouleur_et_gras(`Les produits en croix sont égaux.`)}<br>
						$${texNombre(situations[k].c)}\\times ${situations[k].inc} = ${texNombre(situations[k].a)}\\times ${texNombre(situations[k].b)}$<br>
						${texteEnCouleur_et_gras(`On divise les deux membres par ${texNombre(situations[k].c)}`)}.<br>
						...
						${texteEnCouleur_et_gras(`On simplifie et on calcule.`)}<br>
