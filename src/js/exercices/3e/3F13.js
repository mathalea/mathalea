...
export default function Antecedent_graphique() {
	...
	this.nouvelleVersion = function (numeroExercice) {
		...
		let a, b, c, d, x1, x2, x3, fx1, fx2, fx3, expression_f, numa, dena, numb, denb, ymax,texte,texteCorr;
		...
		if (this.sup == 2) {
			if (randint(1, 4) < 2) { // une fois sur 4 il n'y a qu'un seul antécédent
				...
				while (dena == 0 || denb == 0 || numa == 0) {
					x1 = randint(-4, -1);
					x3 = randint(1, 4);
					sortieHtml ? fx1 = randint(-7, 7) : fx1 = randint(-3, 3);
					fx3 = fx1;
		...
		this.MG32code_pour_modifier_la_figure = `
	        mtg32App.giveFormula2("MG32svg${numeroExercice}", "f", "${expression_f}");
