...
export default function Exercice_Thales () {
  ...
  this.nouvelleVersion = function (numeroExercice) {
    ...
    const y2 = randint(3, 5)
    const x3 = randint(5, 6)
    const y3 = randint(-2, 1)
    let k = (randint(2, 8) * randint(-1, 1, [0])) / 10
    if (this.quatrieme) {
      k = abs(k)
    }
    const dist23 = arrondi(Math.sqrt((x3 - x2) ** 2 + (y3 - y2) ** 2), 1) // calcul des longueurs du triangle principal
    const dist12 = arrondi(Math.sqrt(x2 ** 2 + y2 ** 2), 1)
    const dist13 = arrondi(Math.sqrt(x3 ** 2 + y3 ** 2), 1)
    const dist15 = arrondi(dist13 * abs(k), 2)
    const dist45 = arrondi(dist23 * abs(k), 2)
    let dist35, texte, texteCorr
    const dist14 = arrondi(dist12 * abs(k), 2) // calcul des longueurs demandées à partir

    // On ne garde qu'une approximation au dixième pour l'exercice
    const s45 = texNombrec(dist45) // mise en texte avec 1 chiffres après la virgule pour énoncé
    const s13 = texNombrec(dist13)
    const s12 = texNombrec(dist12)
    const s15 = texNombrec(dist15)
    const s14 = texNombrec(dist14)
    const s23 = texNombrec(dist23)
    if (k < 0) {
    ...
    if (sortieHtml) {
      ...
      if (this.sup == 1) {
        ...
        if (k > 0) {
          texteCorr =
            ...
            `$${texFraction(s1 + s4, s1 + s2)}=${texFraction(
              s1 + s5,
              s1 + s3
            )}=${texFraction(s4 + s5, s2 + s3)}.$` +
            '<br>'
        } else {
          texteCorr =
            'Les droites ' +
      ...
      texteCorr +=
        `$${texFraction(s1 + s4, s12)}=${texFraction(
          s15,
          s13
      ...
      if (this.sup < 3) {
        ...
        this.mg32init = (mtgApp, idDoc) => {
          mtgApp.giveFormula2(idDoc, 'x3', x3)
          mtgApp.giveFormula2(idDoc, 'y2', y2)
          mtgApp.giveFormula2(idDoc, 'y3', y3)
          mtgApp.giveFormula2(idDoc, 'k', k)
          mtgApp.rename(idDoc, 'A\'', s1)
          mtgApp.rename(idDoc, 'B\'', s2)
          mtgApp.rename(idDoc, 'C\'', s3)
          mtgApp.rename(idDoc, 'M\'', s4)
          mtgApp.rename(idDoc, 'N\'', s5)
      ...
      if (this.sup == 2) {
        // niveau 2 : Calcul intermédiaire nécessaire
        texteCorr = `Les droites $(${s4 + s5})$ et $(${s2 + s3})$ sont parallèles.<br>\n\t D\'après la propriété de Thales, on a $${texFraction(
          s1 + s4,
          s1 + s2
        )}=${texFraction(s1 + s5, s1 + s3)}=${texFraction(
          s4 + s5,
          s2 + s3
        )}.$<br>\n\t`
        if (k > 0) {
      ...
      texteCorr += `<br>\n On a donc $${texFraction(
        s1 + s4,
        s12
      )}=${texFraction(s15, s13)}=${texFraction(s45, s2 + s3)}$`
      texteCorr += `<br>\n Soit $${s1 + s4}=${texFraction(
        s15 + '\\times' + s12,
        s13
      )}\\approx${s14}~\\text{cm}$.`
      texteCorr += `<br>\n Et $${s2 + s3}=${texFraction(
        s13 + '\\times' + s45,
        s15
      )}\\approx${s23}~\\text{cm}$.`
