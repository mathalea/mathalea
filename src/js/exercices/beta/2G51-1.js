...
export default function equationcartesienne () {
  ...
  this.titre = titre
  this.consigne = 'Déterminer l\'équation cartésienne de la droite $\\bm{(d)}$'
  this.nbQuestions = 2
      ...
      switch (listeTypeQuestions[i]) { // Suivant le type de question, le contenu sera différent
        case 'cartesienne1':
          ...
          if (this.sup === 1) {
            ...
            if (xu === 0) {
              texteCorr += ` <br>Une équation cartésienne est donc de la forme : $ (d): ${reduireAxPlusB(yu, 0)} ${ecritureAlgebrique(-xA * yu + yA * xu)}=0$ `
            } else {
