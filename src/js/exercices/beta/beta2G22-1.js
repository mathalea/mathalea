  ...
  this.nouvelleVersion = function () {
    ...
    for (let i = 0, r, A, B, O, I, J, j, t, k, l, s, o, ux, uy, xA, yA, xB, yB, nomi, nomj, nomAB, texte, texteCorr, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      if (this.sup === 1) {
        ...
        texte = ` Dans un repère orthonormé $(O,\\vec i,\\vec j)$, représenter le vecteur $\\vec{u}\\begin{pmatrix}${ux} \\\\${uy}\\end{pmatrix}$<br>`
        texte += ` ayant pour origine le point $A\\left(${xA};${yA}\\right)$`

        texteCorr = '<br>On sait qu\'un vecteur mesure un déplacement.'
        texteCorr += `<br> A partir du point $A$,  on trace donc le déplacement correspondant à ${ux} unités horizontalement et ${uy} verticalement.<br>`
