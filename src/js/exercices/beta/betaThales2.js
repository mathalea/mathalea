          ...
          ********************************`);
            ...
            switch (nquestion) { // Chaque question peut être d'un type différent, ici 4 cas sont prévus...
                ...
                case 12: {
                    ...
                    const graph = graphic.getMathalea2DExport(A, B, l);
                ...
                case 14: {
                    ...
                    const graph = graphic.getMathalea2DExport(A, B, C, D, E, F, G, graphic.addSidesPolygon(A, B, C, D), graphic.addSidesPolygon(A, B, E, F), graphic.addSidesPolygon(F, A, D, G));
          ...
          $\\textbf{2.}$ L'image de $${ABCD[0].name}$ est $${EFGH[0].name}$.`
                ...
                case 17: {
                    ...
                    const graph = graphic.getMathalea2DExport(O, A, B, M, N, graphic.addSidesPolygon(O, A, B), // Les segments visibles sont les côtés des deux triangles OAB et OMN
          ...
          `;
                ...
                case 21: {
                    ...
                    // Grandeur.nameAndValue donne un format latex de la forme k = 1.5 cm par exemple
                    // Grandeur.calcul donne une chaine de caractère avec les calculs au format string
                    const aleaDonnees = aleaName([`$${toTex(`${OA.name} = ${OA.toFixed}${OA.unit}`, { suppr1: false })}$`,
          ...
          `;
                ...
                case 22: {
                    ...
                    graphic.clipVisible = true;
                    const graph = graphic.getMathalea2DExport(l1, l2, A, l3);
                    exercice.texte = graph;
                    break;
                }
                case 23: {
...
La rotation est donc de centre $\\color{red}\\fbox{${ABCD[1]}}$ et d'angle $\\color{red}\\fbox{$${angle.name}=${angle.value}$\\degree}$.`;
                ...
                case 24: {
                    ...
                    const graph = graphic.getMathalea2DExport(O, ...polygons);
                    exercice.texte = graph;
                    break;
                }
                case 25: {
                    ...
                    const graph = graphic.getMathalea2DExport(A, B, C, D, E, F, G, H, I, triangle1, triangle2, triangle3);
                    exercice.texte = graph;
                    break;
                }
                case 26: {
                    ...
                    M.showDot();
                    const graph = graphic.getMathalea2DExport(triangle1, graphic.addSidesPolygon(...triangle1), M, triangle2, graphic.addSidesPolygon(...triangle2));
                    exercice.texte = graph;
                    break;
                }
                case 27: {
                    ...
                    const graph = graphic.getMathalea2DExport(triangle1, M, A, B, C, triangle2);
                    exercice.texte = graph;
                    break;
                }
                case 28: {
                ...
                case 29: {
                    ...
                    const graph = graphic.getMathalea2DExport(A, B, C, triangle1);
                    exercice.texte = graph;
                    break;
                }
                case 30: {
                    ...
                    C.showDot();
                    C.showLabel();
                    const graph = graphic.getMathalea2DExport(A, B, C);
                    exercice.texte = graph;
                    break;
                }
                case 31: {
                    ...
                    const aleaDonnees = [];
                    aleaDonnees[0] = aleaName([`${OA.name} = ${OA.toFixed}${OA.unit}`,
                        `${OB.name} = ${OB.toFixed}${OB.unit}`,
                        `${OM.name} = ${OM.abs().toFixed}${OM.unit}`,
                        `${MN.name} = ${MN.abs().toFixed}${MN.unit}`
                    ].map(x => `$ {${toTex2(x)}}$`)).join(', ');
                    aleaDonnees[1] = aleaName([`${OA.name} = ${OA.toFixed}${OA.unit}`,
                        `${OB.name} = ${OB.toFixed}${OB.unit}`,
                        `${OM.name} = ${OM.abs().toFixed}${OM.unit}`,
                        `${MN.name} = ${MN.abs().toFixed}${MN.unit}`
                    ].map(x => `$ {${toTex2(x)}}$`)).join(', ');
                    aleaDonnees[2] = aleaName([`${OA.name} = ${OA.toFixed}${OA.unit}`,
...
${consigne[this.sup3 - 1]}` + '<br>' + graph;
                ...
                case 32: {
                    ...
                    // B.showDot()
                    C.showDot();
                    const graph = graphic.getMathalea2DExport(graphic.addSidesPolygon(A, B, C), B, A, C // , D, E, F
                    );
                    exercice.texte = graph;
                    break;
                }
                case 33: {
                    ...
                    const [A, B, C] = graphic.addNotAlignedPoint();
                    A.showDot();
                    const graph = graphic.getMathalea2DExport(A, B, C, graphic.addSidesPolygon(A, B, C));
                    exercice.texte = graph;
                    break;
                }
                case 34: {
...
${remarque}`;
                ...
                case 36: {
                    ...
                    const graph = graphic.getMathalea2DExport(...triangle.vertices, ...graphic.addSidesPolygon(...triangle.vertices), ...angles.map(x => { x.fillColor = 'red'; x.right = true; return x; }));
...
\end{aligned}$`;
                ...
                case 37: {
                    ...
                    const DEF = graphic.addTriangle(k.toFixed * graphic.distance(A, B), k.toFixed * graphic.distance(B, C), k.toFixed * graphic.distance(A, C));
