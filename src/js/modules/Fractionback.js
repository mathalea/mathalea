...
class Fraction {
  constructor (num, den) {
     ...
     */
    this.n = num || 0
    if (typeof this.n !== 'number' || Number.isNaN(this.n)) throw Error(`Numérateur invalide ${this.n}`)
    /**
     * Dénominateur (1 par défaut)
     * @type {number}
     */
    this.d = den || 1
    if (typeof this.d !== 'number' || Number.isNaN(this.d)) throw Error(`Dénominateur invalide ${this.d}`)
    if (this.d === 0) throw Error('Dénominateur nul impossible')
     ...
     * @type {number}
     */
    this.s = (this.n === 0) ? 0 : unSiPositifMoinsUnSinon(this.n * this.d)

    /**
     * le signe de la fraction : '-' ou '+'
     * @type {string}
     */
    this.signeString = (this.s === -1) ? '-' : (this.s === 0) ? '' : '+'
    ...
    definePropRo(this, 'texFractionSigneeParentheses', () => {
      if (!texFractionSigneeParentheses) texFractionSigneeParentheses = (this.s >= 0) ? this.texFraction : `(${this.texFractionSignee})`
    ...
    definePropRo(this, 'ecritureAlgebrique', () => {
      if (!ecritureAlgebrique) ecritureAlgebrique = this.s === 1 ? '+' + this.texFractionSigneDevant : this.texFractionSigneDevant
    ...
    definePropRo(this, 'ecritureParentheseSiNegatif', () => {
      if (!ecritureParentheseSiNegatif) ecritureParentheseSiNegatif = this.s === 1 ? this.texFractionSigneDevant : '\\left(' + this.texFractionSigneDevant + '\\right)'
     ...
     * @type {number}
     */
    this.valeurDecimale = arrondi(this.n / this.d, 6)
   ...
   */
  fractionEgale (k) {
    return new Fraction(calcul(this.n * k), calcul(this.d * k))
  }
   ...
   */
  sommeFraction (f2) {
    return new Fraction(this.n * f2.d + f2.n * this.d, this.d * f2.d)
  }
  ...
  sommeFractions (...fractions) {
    let s = new Fraction(this.n, this.d)
   ...
   */
  produitFraction (f2) {
    return new Fraction(this.n * f2.n, this.d * f2.d)
  }
  ...
  texProduitFraction (f2) {
    return `${this.texFraction}\\times ${f2.texFraction}=${texFraction(this.n + '\\times' + f2.n, this.d + '\\times' + f2.d)}=${texFraction(this.n * f2.n, this.d * f2.d)}`
  }
   ...
   */
  puissanceFraction (n) {
    return new Fraction(this.n ** n, this.d ** n)
  }
  ...
  produitFractions (...fractions) {
    let p = new Fraction(this.n, this.d)
    for (const f of fractions) {
  ...
  texQuotientFraction (f2) {
    return `${this.texFraction}\\div ${f2.texFraction}=${this.texFraction}\\times ${f2.inverse().texFraction}=${texFraction(this.n + '\\times' + f2.d, this.d + '\\times' + f2.n)}=${texFraction(this.n * f2.d, this.d * f2.n)}`
  }
   ...
   */
  multiplieEntier (n) {
    return new Fraction(n * this.n, this.d)
  }
  ...
  multiplieEntierIrred (n) {
    return new Fraction(n * this.n, this.d).simplifie()
  }
   ...
   */
  entierDiviseIrred (n) {
    return new Fraction(this.n, n * this.d).simplifie()
  }
   ...
   */
  diviseEntierIrred (n) {
    return new Fraction(n * this.d, this.n).simplifie()
  }
   ...
   */
  ajouteEntier (n) {
    return new Fraction(this.n + this.d * n, this.d)
  }
  ...
  entierMoinsFraction (n) {
    return new Fraction(n * this.d - this.n, this.d)
  }
  ...
  estUneSimplification (f2) {
    return this.egal(f2) && (Math.abs(this.n) < Math.abs(f2.n))
  }
   ...
   */
  estIrreductible () {
    return (this.n === this.numIrred && this.d === this.denIrred)
  }
   ...
   */
  egal (f2) {
    return Math.abs((this.n / this.d) - (f2.n / f2.d)) < 1e-10
  }
  ...
  representation (x, y, rayon, depart = 0, type = 'gateau', couleur = 'gray', unite0 = 0, unite1 = 1, scale = 1, label = '') {
    ...
    if (type === 'gateau') {
      ...
      for (k = 0; k < n; k++) {
        for (let j = 0; j < diviseur; j++) {
          for (let h = 0; h < calcul(this.d / diviseur); h++) {
            O = point(x + k * (rayon + 1) + j * rayon / diviseur, y + h * rayon / diviseur)
            C = translation(O, vecteur(rayon / diviseur, 0))
            dep = carre(O, C)
            dep.color = 'black'
