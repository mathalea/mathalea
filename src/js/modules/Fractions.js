...
/**
 * Classe Fraction - Méthodes utiles sur les collections de fractions
...
class ListeFraction {
  constructor (...fractions) {
    ...
    let listetemp = []; let den; let dens = []
    ...
    this.sortFractions = function (liste) { // une fonction pour trier la liste et retourner une liste dans l'ordre croissant
      const fractions = []
      for (let i = 0; i < liste.length; i++) { fractions.push(liste[i]) }
      let changed, tmp
      do {
        changed = false
...
class Fraction {
  constructor (num, den) {
    ...
    if (this.num === 0) { this.signe = 0 } else { this.signe = unSiPositifMoinsUnSinon(this.num * this.den) } // le signe de la fraction : -1, 0 ou 1
    ...
    if (this.signe >= 0) { this.texFractionSigneeParentheses = this.texFraction } else { this.texFractionSigneeParentheses = '(' + this.texFractionSignee + ')' }
    ...
    this.fractionDecimale = function () {
      ...
      if (n5 === n2) { return fraction(this.numIrred, this.fractionDecimale.denIrred) } else if (n5 > n2) { return fraction(this.numIrred * 2 ** (n5 - n2), this.denIrred * 2 ** (n5 - n2)) } else { return fraction(this.numIrred * 5 ** (n2 - n5), this.denIrred * 5 ** (n2 - n5)) }
    }
    ...
    this.superieurstrict = function (f2) {
      if (this.num / this.den > f2.num / f2.den) { return true } else { return false }
    }
    ...
    this.representationIrred = function (x, y, rayon, depart = 0, type = 'gateau', couleur = 'gray', unite0 = 0, unite1 = 1, scale = 1, label = '') {
      const objets = []; let n; let num; let k; let dep; let s; let a; let O; let C
