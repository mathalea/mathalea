...
function reperage_point_du_plan(){
	'use strict';
	Exercice.call(this); // Héritage de la classe Exercice()
	this.titre = "Déterminer les coordonnées d'un point";
	...
	if (sortie_html) {
		...
		window.SVGExist[id_du_div] = setInterval(function() {
			...
			for (let i=0;i<5;i++)	SVG_tracer_point(mon_svg,calcul(20+(points[i][0]-xmin)*w/(xmax-xmin)),calcul(w-(points[i][1]-ymin)*w/(ymax-ymin)),nom[i],'blue')
	...
	for (i=0;i<5;i++) {
	this.liste_questions.push(`Déterminer les coordonnées du point ${nom[i]}.`)
	this.liste_corrections.push(`Les coordonnées du point ${nom[i]} sont  (${points[i][0]};${points[i][1]}).`)
	}
