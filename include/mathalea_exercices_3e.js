...
function Agrandissement_reduction(){
	'use strict';
		...
		this.liste_questions = [];
		this.liste_corrections = [];
		let texte,texte_corr,r,h,h2,m;
		if (sortie_html) {
			this.type_exercice = 'MG32';
			this.taille_div_MG32 = [600,700];
			let codeBase64
			let choix=randint(2,2)
			switch (choix) {
				case 1 :
					m=randint(10,20)
					...
					this.MG32codeBase64 = codeBase64
					this.MG32code_pour_modifier_la_figure = `
							mtg32App.giveFormula2("MG32svg${numero_de_l_exercice}", "m", "${m}");
							mtg32App.calculate("MG32svg${numero_de_l_exercice}");
							mtg32App.display("MG32svg${numero_de_l_exercice}");
							` 	
						break;
				case 2 :
					r=randint(12,35)/10
					h=randint(12,20)/2
					h2=randint(3,Math.floor(h)-1)
					codeBase64 ="TWF0aEdyYXBoSmF2YTEuMAAAABI+TMzNAANmcmH###8BAP8BAAAAAAAAAAAFHAAAAtIAAAAAAAAAAAAAAAAAAACA#####wAAAAEACkNDYWxjQ29uc3QA#####wACcGkAFjMuMTQxNTkyNjUzNTg5NzkzMjM4NDb#####AAAAAQAKQ0NvbnN0YW50ZUAJIftURC0Y#####wAAAAEACkNQb2ludEJhc2UA#####wAAAAAADwABTwDAMQAAAAAAAMA1AAAAAAAABQABQGHQAAAAAABAU+FHrhR64v####8AAAABABRDRHJvaXRlRGlyZWN0aW9uRml4ZQD#####AQAAAAAQAAABAAEAAAABAT#zMzMzMzMz#####wAAAAEAD0NQb2ludExpZURyb2l0ZQD#####AQAAAAALAAJJMQDAEAAAAAAAAEAQAAAAAAAABQABQEA#######oAAAAC#####wAAAAEACUNEcm9pdGVBQgD#####AQAAAAANAAABAAEAAAABAAAAA#####8AAAABABZDRHJvaXRlUGVycGVuZGljdWxhaXJlAP####8BAAAAABAAAAEAAQAAAAEAAAAE#####wAAAAEACUNDZXJjbGVPQQD#####AQAAAAABAAAAAQAAAAP#####AAAAAQAQQ0ludERyb2l0ZUNlcmNsZQD#####AAAABQAAAAb#####AAAAAQAQQ1BvaW50TGllQmlwb2ludAD#####AQAAAAALAAJKMQDAKAAAAAAAAMAQAAAAAAAABQACAAAAB#####8AAAACAAdDUmVwZXJlAP####8AgICAAQEAAAABAAAAAwAAAAgAAAAAAAABAAAAAAAAAAAAAAABAAAAAAAAAAAAAAABP#AAAAAAAAAAAAABP#AAAAAAAAAAAAACAP####8BAAAAAQ8AAk8yAQUAAUBy6AAAAAAAQHZ4UeuFHrgAAAACAP####8AAAAAAQ8AAk8zAQUAAUCItAAAAAAAQHNIUeuFHrgAAAADAP####8BAAAAABAAAAEAAQAAAAsBP#MzMzMzMzMAAAACAP####8AAAAAAQ8AAk80AQUAAUCI3AAAAAAAQHn4UeuFHrgAAAADAP####8BAAAAABAAAAEAAQAAAA0BP#MzMzMzMzMAAAADAP####8BAAAAARAAAAEAAQAAAAoAP#AAAAAA
					...
					texte_corr +=num_alpha(1)+` Le volume du cône est $\\dfrac{\\text{Aire de la base}}{3}\\times \\text{hauteur}$ cm${exposant(3)} $= \\dfrac{${tex_nombrec(r*r)}\\pi}{3} \\times ${tex_nombre(h)}$ cm${exposant(3)} $= \\dfrac{${tex_nombrec(r*r*h)}}{3}\\pi$ cm${exposant(3)} $\\approx ${tex_nombrec(arrondi(r*r*h*Math.PI/3))}$ cm${exposant(3)}.<br>`
					texte_corr +=num_alpha(2)+ ` La section est une réduction de la base de coefficient $\\dfrac{${h2}}{${tex_nombre(h)}}`
					if (!Number.isInteger(h) ||pgcd(h2,h)>1 ) texte_corr +=`=${tex_fraction_reduite(h2*10,h*10)}$.<br>`
					else texte_corr +=`.$<br>`
					texte_corr +=`Dans une réduction de coefficient k, les aires sont multipliés par k ${exposant(2)}.<br>`
					texte_corr +=`Donc son aire est $\\left(${tex_fraction_reduite(h2*10,h*10)}\\right)^2 \\times ${tex_nombrec(r*r)}\\pi$ cm${exposant(2)} $=${tex_fraction_reduite(arrondi(h2*h2*100*r*r) ,arrondi(h*h*100))}\\pi$ cm${exposant(2)} $\\approx${tex_nombrec(arrondi(h2*h2*r*r*Math.PI/h/h))}$ cm${exposant(2)}.<br>`
					...
					texte_corr += `$VolumeCône - VolumeCôneRéduit$<br>Soit : <br>$\\dfrac{${tex_nombrec(r*r*h)}}{3}\\pi$ cm${exposant(3)}$ - \\left(${tex_fraction_reduite(h2*10,h*10)}\\right)^3 \\times \\dfrac{${tex_nombrec(r*r*h)}}{3}\\pi$ cm${exposant(3)} `
					texte_corr += `$ = \\left(1-\\dfrac{${fraction_simplifiee(h2*10,h*10)[0]**3}}{${fraction_simplifiee(h2*10,h*10)[1]**3}}\\right)\\times \\dfrac{${tex_nombrec(r*r*h)}}{3}\\pi$ cm${exposant(3)} `
					texte_corr += `$ = \\dfrac{${fraction_simplifiee(h2*10,h*10)[1]**3-fraction_simplifiee(h2*10,h*10)[0]**3}}{${fraction_simplifiee(h2*10,h*10)[1]**3}}\\times \\dfrac{${tex_nombrec(r*r*h)}}{3}\\pi$ cm${exposant(3)} `
					texte_corr += `$ \\approx ${tex_nombrec(arrondi((fraction_simplifiee(h2*10,h*10)[1]**3-fraction_simplifiee(h2*10,h*10)[0]**3)*r*r*h*Math.PI/(fraction_simplifiee(h2*10,h*10)[1]**3*3)))}$ cm${exposant(3)}<br>`
					texte_corr +=`Le point O peut être déplacé, et l'angle de vue changé avec l'angle &#x3C6; `
					 ...
					 this.MG32code_pour_modifier_la_figure = `
							 ...
							 mtg32App.giveFormula2("MG32svg${numero_de_l_exercice}", "h'", "${h2}");							 
							 mtg32App.calculate("MG32svg${numero_de_l_exercice}");
							 mtg32App.display("MG32svg${numero_de_l_exercice}");
							 ` 	
					 break;
