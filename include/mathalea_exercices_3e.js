...
function Agrandissement_reduction(){
	'use strict';
		...
		this.liste_questions = [];
		this.liste_corrections = [];
		let texte,texte_corr,r,h1,h2,m;
		if (sortie_html) {
			this.type_exercice = 'MG32';
			...
			switch (choix) {
				...
				case 2 :
					...
					texte_corr += `Donc le volumle du cône de hauteur OA' est : $\\left(${tex_fraction_reduite(h2*10,h1*10)}\\right)^3 \\times \\dfrac{${tex_nombrec(r*r*h1)}}{3}\\pi$ cm${exposant(3)} $\\approx ${tex_nombrec(arrondi(Math.PI*h2**3*r*r*h1/h1**3/3))}$ cm${exposant(3)} '.<br>`
