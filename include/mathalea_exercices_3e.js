...
function Agrandissement_reduction() {
	...
	this.nouvelle_version = function (numero_de_l_exercice) {
		...
		if (sortie_html) {
			...
			else if (this.sup == 2) choix = 4
			else choix = randint(1, 4)
			switch (choix) {
				...
				case 4: // Un tronc de cône étant donné (seau), calcul de la hauteur du cône dont il est issu, de son volume, puis du volume du seau. Lecture graphique du volume d'eau à mi hauteur et calcul de ce volume
					...
					texte_corr += `D'où SH $=\\dfrac{${tex_nombrec(r * h3)}}{${tex_nombrec(r - r2)}}=${tex_nombre(h1)}$ dm.<br>`
					...
					texte_corr += `Donc le volumle du cône de hauteur SB est : $\\left(${tex_fraction_reduite(arrondi(r2 * 10), arrondi(r * 10))}\\right)^3 \\times \\dfrac{${tex_nombrec(r * r * h1)}}{3}\\pi$ dm${exposant(3)} $\\approx ${tex_nombrec(arrondi(Math.PI * h2 ** 3 * r * r / h1 ** 2 / 3))}$ dm${exposant(3)} '.<br>`
