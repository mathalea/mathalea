...
function Image_antecedent_graphique(){
	...
	this.nouvelle_version = function(numero_de_l_exercice){
		...
		let a = randint(-3,3,[0]);
		let b = randint(-3,3,[0]);
		let expression_f = `${a}*x${ecriture_algebrique(b)}`;
		console.log(expression_f)
		let x1 = randint(-3,-1);
		let x2 = randint(0,4);

		texte = `Déterminer par lecture graphique l'image de ${x1} et de ${x2}.`
		texte_corr = `L'image de ${x1} est ${a*x1+b}.<br>`
		texte_corr += `L'image de ${x2} est ${a*x2+b}.`
