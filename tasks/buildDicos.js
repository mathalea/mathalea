...
console.log(jsDir, exercicesDir)
...
for (const file of exercicesList) {
  ...
  try {
    ...
    const promesse = import(`../${path.relative('.', file).replaceAll('\\', '/')}`).then(
      (module) => {
        if (!module.titre) {
      ...
      () => {
        console.log('Je suis passé par le catch de promise et non pas le catch du try...')
    ...
    console.log('Je suis passé par le catch du try, et non le catch de promise...')
    // console.log(`${error} dans ${file}`)
    // Pour les fichiers TS, pour ne pas les compiler, on passe par une regex
    // ça marche pas pour ce fichier, probablement parce qu'il importe du css et qu'on a pas les loader webpack
    // on passe à l'ancienne méthode qui fouille dans le code simport { amcReady } from '../src/js/exercices/3e/3G21';
...
Promise.all(promesses).then(
  () => {
    ...
    const mdDir = path.resolve('.', 'src')
    // Réperetoire père pour les csv
    const csvDir = path.resolve('.', 'src/csv')
    ...
    const fin = Date.now()
    console.log(`${path.resolve('.', 'tasks', 'buildDicos.js')} terminé en ${fin - debut}ms`)
  }
)
