...
import {liste_de_question_to_contenu,randint,sp,choice,range1,combinaison_listes,ecriture_algebrique,ecriture_parenthese_si_negatif,mise_en_evidence,liste_des_diviseurs} from "/modules/outils.js"
/**
  ...
  this.nouvelle_version = function () {
      ...
      i < this.nb_questions && cpt < 50;)      
    {
      ...
      switch (liste_type_de_questions[i]) {
        ...
        case 2: //a + b²*c   
          texte = `$${a} ${sp()} + ${sp()} ${ecriture_parenthese_si_negatif(p)}^2 \\times ${ecriture_parenthese_si_negatif(c)}$`;
          texte_corr = `$${a} ${sp()} + ${sp()} ${mise_en_evidence(ecriture_parenthese_si_negatif(p) + '^2', 'blue')} \\times ${ecriture_parenthese_si_negatif(c)}
            =${a} ${sp()} + ${mise_en_evidence(ecriture_parenthese_si_negatif(p*p) + '\\times' + ecriture_parenthese_si_negatif(c), 'blue')}
            =${a + ecriture_algebrique(p*p*c)}
            =${a + p*p*c}$`;
            break;     

        case 3: //a²+b+c*d 
        texte = `$${ecriture_parenthese_si_negatif(a)}^2 ${sp()} + ${sp()}  ${ecriture_parenthese_si_negatif(b)} ${sp()} ${ecriture_algebrique(c)} \\times ${ecriture_parenthese_si_negatif(d)}$`;
        texte_corr = `$${mise_en_evidence(ecriture_parenthese_si_negatif(a) + '^2','blue')} ${sp()} ${ecriture_algebrique(b)} ${sp()}  ${ecriture_algebrique(c)} \\times ${ecriture_parenthese_si_negatif(d)}
           =${a*a} ${ecriture_algebrique(b)} ${sp()} + ${sp()}  ${mise_en_evidence((ecriture_parenthese_si_negatif(c) + "\\times" + ecriture_parenthese_si_negatif(d)),'blue') }
        ...
        case 4: //a²*(b+c)
          ...
          texte_corr = `$${mise_en_evidence(ecriture_parenthese_si_negatif(n) + '^2','blue')}  \\times ( ${b+ecriture_algebrique(c)})
             =${n*n} \\times ( ${mise_en_evidence(b + ecriture_algebrique(c), 'blue')})
             =${n*n} \\times ${ecriture_parenthese_si_negatif(b+c)}
            =${n*n*(b+c)}$`;
            break;      

        case 5: //m*(n²+p*n)
            texte = `$${m} \\times ( ${ecriture_parenthese_si_negatif(n)}^2${ecriture_algebrique(p)}\\times${ecriture_parenthese_si_negatif(n)})$`;
            texte_corr = `$${m} \\times ( ${mise_en_evidence(ecriture_parenthese_si_negatif(n)+'^2','blue')} ${ecriture_algebrique(p)}\\times${ecriture_parenthese_si_negatif(n)})
               ...
               =${m}\\times ${ecriture_parenthese_si_negatif( n*n + p*n)}
              =${m*(n*n+p*n)}$`;
              break;    

        case 6: //(a+b+n²)*d 
              texte = `$(${a} ${ecriture_algebrique(b)} + ${ecriture_parenthese_si_negatif(n)}^2 ) \\times ${ecriture_parenthese_si_negatif(d)}$`;
              texte_corr = `$(${a} + ${ecriture_parenthese_si_negatif(b)} + ${mise_en_evidence(ecriture_parenthese_si_negatif(n) +'^2','blue')}  ) \\times ${ecriture_parenthese_si_negatif(d)}
                 =(${mise_en_evidence((a + ecriture_algebrique(b) + ecriture_algebrique(n*n)),'blue') }) \\times ${ecriture_parenthese_si_negatif(d)}
                 =${a + b + n*n} \\times ${ecriture_parenthese_si_negatif(d)}
                =${(a+b+n*n)*d}$`;
                break;   

        case 7: //n²*(a+b+c)
                texte = `$${ecriture_parenthese_si_negatif(n)}^2 \\times ( ${a+ecriture_algebrique(b)+ecriture_algebrique(c)})$`;
                texte_corr = `$${mise_en_evidence(ecriture_parenthese_si_negatif(n) +'^2','blue') } \\times ( ${a} ${ecriture_algebrique(b)} ${ecriture_algebrique(c)})
