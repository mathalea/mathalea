...
import {liste_de_question_to_contenu,randint,combinaison_listes,troncature,calcul,tex_nombre,tex_nombrec,tex_prix,carreParfait} from "/modules/outils.js";
/**
...
miracd = troncature(racd-troncature(racd,2),3);
  ...
  texte_corr += `<br>$\\phantom{123456}\\sqrt{${tex_nombrec(a*a+b*b)}}$`;

  if (carreParfait(calcul(100*s))) {texte_corr += `$\\phantom{1}=\\phantom{1}${tex_nombre(racs)}$  (qui est la valeur exacte de $\\sqrt{${tex_nombrec(a*a+b*b)}}$)`;
}else{
  if (1000*miracs < 5) {
  ...
  if (carreParfait(calcul(100*d))) {texte_corr += `$\\phantom{1}=\\phantom{1}${tex_nombre(racd)}$  (qui est la valeur exacte de $\\sqrt{${tex_nombrec(a*a-b*b)}}$)`;
}else{
  if (1000*miracd < 5) {
