...
import { premierMultipleSuperieur,liste_de_question_to_contenu,tex_nombre2,arrondi,sp, mise_en_evidence,combinaison_listes,tex_fraction, randint,num_alpha, choice, prenom,prenomSexe,premiere_lettre_en_majuscule ,prenomM,prenomF ,ppcm,calcul} from "/modules/outils.js"
    ...
    for (let i = 0, texte, texte_corr, x, y, z, total, a, b, c, n = 2, k, cpt = 0; i < this.nb_questions && cpt < 50;) {
      ...
      switch (liste_type_de_questions[i]) { // Chaque question peut être d'un type différent, ici 4 cas sont prévus...
        case 'partage':
          ...
          texte += ` et ${prenoms[n - 1]} se partagent $${total}$ ${objets[index%5]} dans le ratio $~${x}~:~${y}$`
          if (n == 3) {
            texte += `$~:~${z}$`
          }
          texte += `.<br>Combien de ${objets[index%5]} chaque enfant reçoit-il ?`
          if (n == 2) {
            texte_corr += `Si les enfants se partageaient $${x}+${y}=${x + y}$ ${objets[index%5]} alors ${prenoms[0]} en aurait $${x}$ et ${prenoms[1]} en aurait $${y}$.`
            ...
            texte_corr += `Conclusion : ${prenoms[0]} aura $${k * x}$ ${objets[index%5]} et ${prenoms[1]} en aura $${k * y}$.`

          }
          else {
            texte_corr += `Si les enfants se partageaient $${x}+${y}+${z}=${x + y + z}$ ${objets[index%5]} alors ${prenoms[0]} en aurait $${x}$, ${prenoms[1]} en aurait $${y}$ et ${prenoms[2]} en aurait $${z}$.`
            texte_corr += `<br>Mais il y a $${total}$ ${objets[index%5]}, soit $${mise_en_evidence(k)}\\times ${x + y + z}$ ${objets[index%5]}.<br>`
            ...
            texte_corr += `Conclusion : ${prenoms[0]} aura $${k * x}$ ${objets[index%5]}, ${prenoms[1]} en aura $${k * y}$ et  ${prenoms[2]} en aura $${k * z}$.`
          }
          break
        case 'mélange':
            x=randint(1,3)
            ...
            prenoms[0]=quidam[0][0]

            if (n==2) { // On mélange du sirop et de l'eau
                texte+=`${prenoms[0]} prépare un sirop à l'eau pour ses amis. ${premiere_lettre_en_majuscule(article)} mélange du sirop ${sirops[index%5]} et de l'eau dans le ratio $~${x}~:~${y}$.<br>`
           if (choice([true,false])){
                ...
                texte_corr+=`Si ${prenoms[0]} mélange selon le ratio donné $${x}\\text{ cL} $ de sirop ${sirops[index%5]} et $${y}\\text{ cL} $ d\'eau ${article} obtiendra $${x+y}\\text{ cL} $ de mélange.<br>`
              ...
              texte_corr+=`${prenoms[0]} doit ajouter un volume d\'eau de : $${tex_fraction(y+'\\times'+k*x,x)}=${y*k}\\text{ cL} $.`
     }
            }
            else { // On mélange du sirop, du jus de fruit et de la limonade.
              texte+=`${prenoms[0]} prépare un coktail à base de sirop  ${sirops[index%5]}, de jus ${jusdefruit[index2%5]} et d\'eau gazeuse pour ses amis. ${premiere_lettre_en_majuscule(article)} mélange les trois ingédients dans le ratio $~${x}~:~${y}~:~${z}$.<br>`
              if (choice([true,false])){
              ...
              texte_corr+=`Si ${prenoms[0]} mélange selon le ratio donné $${x}\\text{ cL} $ de sirop ${sirops[index%5]}, $${y}\\text{ cL} $ de jus ${jusdefruit[index2%5]} et $${z}\\text{ cL} $ d\'eau gazeuse ${article} obtiendra $${x+y+z}\\text{ cL} $ de coktail.<br>`
              ...
              texte_corr+=`Donc pour cela, ${article} doit mélanger $${mise_en_evidence(k)}\\times ${x}\\text{ cL} $$=${k*x}\\text{ cL} $ de sirop ${sirops[index%5]}, $${mise_en_evidence(k)}\\times ${y}\\text{ cL} $$=${k*y}\\text{ cL} $ de jus ${jusdefruit[index2%5]} et $${mise_en_evidence(k)}\\times ${z}\\text{ cL} $$=${k*z}\\text{ cL} $ d\'eau gazeuse.`
                ...
                texte_corr+=`${prenoms[0]} en déduit que le volume de jus ${jusdefruit[index2%5]} est : $${tex_fraction(k*x+'\\times'+y,x)}\\text{ cL}=${y*k}\\text{ cL}$.<br>`
        ...
        if (n==3) {
          ...
          total=(x+y+z)*k
          texte+=`${prenoms[0]} veut faire des sablés bretons. Pour cela ${article} doit réaliser un mélange de farine, de sucre et de beurre selon le ratio $~${x}~:~${y}~:~${z}$.<br>`
          ...
          texte_corr+=`On en déduit que ${prenoms[0]} devra utiliser $${mise_en_evidence(k)}\\times ${x}\\text{ g}=${k*x}\\text{ g}$ de farine et $${mise_en_evidence(k)}\\times ${y}\\text{ g}=${k*y}\\text{ g}$ de sucre.<br>`
          texte_corr+=`${num_alpha(1)} La masse de "sable" sera donc : $${k*x}\\text{ g} + ${k*y}\\text{ g} +${k*z}\\text{ g} =${total}\\text{ g}$.`
            ...
            k=randint(2,6)
            total=(x+y)*k
            texte+=`${prenoms[0]} veut réaliser une vinaigrette. Pour cela ${article} mélange du vinaigre et de l'huile d'olive selon le ratio $~${x}~:~${y}$.<br>`
            ...
            texte+=`${num_alpha(1)} Quel sera le volume de vinaigrette, ${prenoms[0]} réalisera-t-${article} ?`
