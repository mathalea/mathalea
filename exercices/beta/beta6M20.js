import { longueur, segment, mathalea2d, afficheLongueurSegment, codageAngleDroit, pointSurSegment, polygoneAvecNom, triangle2points1hauteur, point, rotation } from '../../modules/2d.js';
import { choisit_lettres_differentes, combinaison_listes_sans_changer_ordre } from '../../modules/outils.js';
...
export default function Aire_de_triangles() {
  ...
  this.nouvelle_version = function (numero_de_l_exercice) {
    ...
    nom=choisit_lettres_differentes(4*this.nb_questions,'Q')
    ...
    for (let i=0;i<this.nb_questions;i++){
        ...
        ymin=Math.min(A.y,B.y,C.y,H.y)-1.5
        ymax=Math.max(A.y,B.y,C.y,H.y)+1.5
        objets_enonce.push(polynom[0],polynom[1],hauteurpoly,afficheLongueurSegment(B,A,'black',1),afficheLongueurSegment(A,C,'black',0.5),afficheLongueurSegment(C,B,'black',0.5),afficheLongueurSegment(C,H,'black',0.3),codageAngleDroit(A,H,C))
