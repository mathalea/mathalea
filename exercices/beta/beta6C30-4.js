...
export default function Placer_la_virgule() {
    ...
    this.consigne=`Placer la virgule dans le résultat en ajoutant éventuellement des zéros et en supprimant ceux qui ne servent pas`
    ...
    let liste_type_de_questions = combinaison_listes(type_de_questions_disponibles, this.nb_questions)
      ...
      for (let i = 0, texte, texte_corr,coef,nombre,nombreentier,resultat,exposant, cpt = 0; i < this.nb_questions && cpt < 50;) {
        ...
        switch (liste_type_de_questions[i]) { // Chaque question peut être d'un type différent, ici 4 cas sont prévus...
          case 1:
               texte= `$${tex_nombrec(nombre)} \\times ${tex_nombrec(10**coef)} = ${tex_nombrec(nombreentier)}$`
               texte_corr=`Quand on multiplie un nombre par $${tex_nombrec(10**coef)}$ chaque chiffres devient $${tex_nombrec(10**(-coef))}$ fois plus petit :<br>$${tex_nombrec(nombre)} \\times ${tex_nombrec(10**coef)} = ${tex_nombrec(Math.floor(resultat))}${mise_en_evidence(',')}${tex_nombrec(resultat-Math.floor(resultat)).replace('0,','')}$`
          break;
