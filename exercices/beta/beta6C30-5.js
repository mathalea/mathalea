...
export default function Placer_la_virgule() {
    "use strict"
    Exercice.call(this)
    this.titre = "Multiplication par 0,1 ; 0,01 ; 0,001 (calculer le résultat)";
    ...
    let liste_type_de_questions = combinaison_listes(type_de_questions_disponibles, this.nb_questions)
      ...
      for (let i = 0, texte, texte_corr,coef,nombre,nombreentier,resultat,exposant, cpt = 0; i < this.nb_questions && cpt < 50;) {
        ...
        switch (liste_type_de_questions[i]) { // Chaque question peut être d'un type différent, ici 4 cas sont prévus...
          case 1:
               texte= `$${tex_nombrec(nombre)} \\times ${tex_nombrec(10**coef)} =$`
               texte_corr=`Quand on multiplie un nombre par $${tex_nombrec(10**coef)}$ chaque chiffres devient $${tex_nombrec(10**(-coef))}$ fois plus petit :<br>$${tex_nombrec(nombre)} \\times ${tex_nombrec(10**coef)} = ${mise_en_evidence(tex_nombrec(resultat),'blue')}$`
          break;
